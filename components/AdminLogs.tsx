...
import { Activity, AlertTriangle, CheckCircle, Info, Trash2 } from 'lucide-react';
...
export const AdminLogs: React.FC<AdminLogsProps> = ({ logs, onClear }) => {
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden animate-fade-in flex flex-col h-[calc(100vh-140px)]">
      ...
      <div className="overflow-y-auto flex-1 p-0 bg-slate-950 font-mono text-sm">
        <div className="divide-y divide-slate-800/50">
           {logs.length === 0 ? (
             <div className="p-8 text-center text-slate-600 text-xs">No active logs available.</div>
           ) : (
             logs.map(log => (
           ...
           <div className="p-3 text-slate-600 text-xs animate-pulse border-t border-slate-800/50">Listening for new events...</div>
        </div>
      </div>
    </div>
  );
